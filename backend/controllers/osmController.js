...
const startPoint = "85.33125,27.68999"; // Replace with your starting point coordinates
...
//   module.exports=calculateDistances;
  ...
  async function calculateDistances(latitude,longitude, vehicleType) {
    try {
      ...
      for (const desiredLocation of parkingLocations) {
        ...
        let availableSpaces= await hasAvailableSpaces(desiredLocation, vehicleType)
  ...
  if(parkingLocation.parkingType=="regulated"){
   ...
   // console.log("Entered UnRegulated")
    let bikeCapacity, carCapaity;
    const response= await axios({
      method:"get",
      url:"http://192.168.100.160:8080/run"
    })
   // console.log(response.data);
    carCapaity=response.data.Car;
    bikeCapacity=response.data.Bike;
